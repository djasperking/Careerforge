...
import { submitOfferForReview, reopenMyOffer, setMyOfferPublished } from "../actions";
  ...
  return (
    <div className="mb-6 rounded-lg border bg-card p-4">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <span className="text-xs text-muted-foreground">Your share of each booking: {revenueSharePercent}%</span>
      </div>
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        {(reviewStatus === "DRAFT" || reviewStatus === "CHANGES_REQUESTED") ? (
          <Button size="sm" disabled={busy} onClick={() => run(() => submitOfferForReview(offerId))}>
            Submit for review
          </Button>
        ) : null}
        {reviewStatus === "APPROVED" && !isLive ? (
          <Button size="sm" disabled={busy} onClick={() => run(() => setMyOfferPublished(offerId, true))}>
            Publish
          </Button>
        ) : null}
        {reviewStatus === "APPROVED" && isLive ? (
          <Button size="sm" variant="outline" disabled={busy} onClick={() => run(() => setMyOfferPublished(offerId, false))}>
            Unpublish
          </Button>
        ) : null}
        {(reviewStatus === "APPROVED" || reviewStatus === "CHANGES_REQUESTED") ? (
          <Button size="sm" variant="ghost" disabled={busy} onClick={() => run(() => reopenMyOffer(offerId))}>
            Move back to draft
          </Button>
        ) : null}
      </div>
