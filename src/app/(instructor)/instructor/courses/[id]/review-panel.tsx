...
import { submitCourseForReview, reopenMyCourse, setMyCoursePublished } from "../../actions";
    ...
    <div className="mb-6 rounded-lg border bg-card p-4">
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        {(reviewStatus === "DRAFT" || reviewStatus === "CHANGES_REQUESTED") ? (
          <Button size="sm" disabled={busy} onClick={() => run(() => submitCourseForReview(courseId))}>
            Submit for review
          </Button>
        ) : null}

        {reviewStatus === "APPROVED" && !isLive ? (
          <Button size="sm" disabled={busy} onClick={() => run(() => setMyCoursePublished(courseId, true))}>
            Publish course
          </Button>
        ) : null}
        {reviewStatus === "APPROVED" && isLive ? (
          <Button size="sm" variant="outline" disabled={busy} onClick={() => run(() => setMyCoursePublished(courseId, false))}>
            Unpublish
          </Button>
        ) : null}

        {(reviewStatus === "APPROVED" || reviewStatus === "CHANGES_REQUESTED") ? (
          <Button size="sm" variant="ghost" disabled={busy} onClick={() => run(() => reopenMyCourse(courseId))}>
            Move back to draft
          </Button>
        ) : null}
      </div>
