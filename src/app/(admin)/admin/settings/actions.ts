...
const schema = z.object({
  ...
  cvOneTimePrice: z.coerce.number().min(0).max(10_000_000),
  ...
  payoutMinimumNaira: z.coerce.number().min(0).max(100_000_000),
...
export async function saveSettings(_prev: SettingsState, formData: FormData): Promise<SettingsState> {
  ...
  const entries: [string, unknown][] = [
    ...
    ["cv.oneTimePriceCents", Math.round(d.cvOneTimePrice * 100)],
    ...
    ["payouts.holdDays", d.payoutHoldDays],
    ["payouts.minimumCents", Math.round(d.payoutMinimumNaira * 100)],
